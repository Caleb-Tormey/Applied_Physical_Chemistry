:::{code-cell} python
:tags: ["remove-input"]
:label: interactive_collision_elastic
from IPython.display import HTML, display
import uuid

def create_elastic_collision_widget():
    ...
    <div id="{unique_id}_container" style="font-family: 'Arial', sans-serif; max-width: 800px; margin: 20px auto; border: 1px solid #ddd; padding: 25px; border-radius: 12px; background: #fff; box-shadow: 0 4px 15px rgba(0,0,0,0.1);">
        <h3 style="text-align: center; color: #2c3e50; margin-top: 0;">Collision Simulator: Elastic vs. Inelastic</h3>
        
        <div style="text-align: center; margin-bottom: 15px;">
            <label style="font-weight: bold; margin-right: 10px;">Collision Type:</label>
            <select id="{unique_id}_type" style="padding: 5px; border-radius: 4px;">
        ...
        <div style="text-align: center; background: #f8f9fa; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
            <svg id="{unique_id}_svg" width="600" height="120" viewBox="0 0 600 120">
                ...
                <text id="{unique_id}_v1_label" x="70" y="50" text-anchor="middle" font-size="12" font-weight="bold">v=1.0</text>
                <text id="{unique_id}_v2_label" x="420" y="50" text-anchor="middle" font-size="12" font-weight="bold">v=0.0</text>
            </svg>
        </div>

        <div style="display: flex; justify-content: space-around; gap: 20px;">
            <div style="flex: 1; text-align: center; border-right: 1px solid #eee;">
                <h4 style="margin: 0 0 10px 0; color: #2c3e50;">Total Momentum ($p$)</h4>
                <div style="font-size: 1.2em; font-family: monospace;" id="{unique_id}_p_val">40.0</div>
            ...
            <div style="flex: 1; text-align: center;">
                <h4 style="margin: 0 0 10px 0; color: #2c3e50;">Total Kinetic Energy ($KE$)</h4>
                <div style="font-size: 1.2em; font-family: monospace;" id="{unique_id}_ke_val">20.0</div>
                <div style="width: 100%; background: #eee; height: 10px; border-radius: 5px; margin-top: 5px;">
        ...
        <div style="text-align: center; margin-top: 25px;">
            <button id="{unique_id}_reset" style="padding: 10px 20px; cursor: pointer; border-radius: 5px; border: none; background: #34495e; color: white; font-weight: bold;">Reset & Run</button>
        </div>
        ...
        <script>
            (function() {{
                ...
                const typeSelect = document.getElementById('{unique_id}_type');
                ...
                function updateDisplay(cv1, cv2) {{
                    const pTotal = (m1 * cv1) + (m2 * cv2);
                    const keTotal = (0.5 * m1 * cv1**2) + (0.5 * m2 * cv2**2);
                ...
                function animate() {{
                    if (!collided && (pos1 + 40 >= pos2)) {{
                        if (typeSelect.value === "inelastic") {{
                            const vFinal = (m1 * v1 + m2 * v2) / (m1 + m2);
                            v1 = vFinal; v2 = vFinal;
                        }} else {{
                            // Elastic Collision: Swap velocities (since masses are equal)
                            let temp = v1;
                            v1 = v2;
                            v2 = temp;
                        }}
...
display(create_elastic_collision_widget())
:::