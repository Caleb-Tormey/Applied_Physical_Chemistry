...
- Derivative of a constant $f\left(x\right) = a$, $f'\left(x\right) =\frac{d f\left(x\right)}{d x} = 0$
    Because a constant doesn't change over any range (duh it's constant), we know that the slope will be zero. So if $f\left(x\right) = a$ then $f'\left(x\right) =\frac{d f\left(x\right)}{d x} = 0$
