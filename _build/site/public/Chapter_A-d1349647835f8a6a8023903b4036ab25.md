...
- Derivative using power rule where something has the form $f\left(x\right) = a x^n$ where $n$ can be any number positive or negative. (we will often see $n$ be integer value, however,  isn't required) The power rule is defined in equation {eq}`power_rule`
:::{math}
:label: power_rule
\frac{df}{dx}=\frac{d}{dx}ax^n =  a n x^{n-1} 
:::
